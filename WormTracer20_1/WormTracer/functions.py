...
def make_theta_from_xy(x, y):
  ...
  for t in range(theta.shape[0] - 1):
    ...
    theta[t + 1, plot_n // 2] = theta[t + 1, plot_n // 2] - np.sign(gap) * 2 * pi * (
      (abs(gap) + pi) // (2 * pi)
    )
...
def calc_cap_span(image_info, plot_n, s_m=8000):
  ...
  try:
    ...
    cap_span = int(
      s_m
      * free_memory
      / (image_info["image_shape"][1] * image_info["image_shape"][2] * (plot_n - 1))
    )
  ...
  try:
    ...
    for i in range(num_span.shape[0]):
      use_points = np.append(
        use_points,
        np.linspace(nont_end[i], nont_ini[i], num_span[i] + 2 - one_span[i], dtype=int),
      )
...
def body_axis_function(body_ratio, plot_n, base=0.5):
  ...
  n = 1 / base - 1
  body_axis_weight = (
    n * (torch.sigmoid(x + body_ratio // 2) + torch.sigmoid(-x + body_ratio // 2) - 1)
    + 1
  ) / (n + 1)
...
def worm_width_all(
  ...
  width = alpha * torch.sqrt(
    ...
    * (1 + (2 * gamma_e) * delta_sigmoid - (2 * gamma_e) * delta_sigmoid * worm_x_abs)
  )
  return width
...
def make_model_image(cent_x, cent_y, theta, unitLength, image_info, params):
  ...
  x = torch.cat(
    (
      ...
      torch.cumsum(unitLength.reshape((T, 1)).to(device) * torch.cos(theta), dim=1),
    ),
    dim=1,
  )
  x = (
  ...
  y = torch.cat(
    (
      torch.zeros((T, 1)).to(device),
      torch.cumsum(unitLength.reshape((T, 1)).to(device) * torch.sin(theta), dim=1),
    ),
    dim=1,
  )
  y = (
