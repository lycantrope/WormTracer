...
Original file is located at
    https://colab.research.google.com/drive/1XGnzc7mRCKL9DmbXe_ij9GutF1ZAoZGu
"""
...
def get_property(filenames, rescale):
  im = cv2.imread(filenames[0], cv2.IMREAD_GRAYSCALE)
  if np.any((0<im)*(im<255)):
  ...
  white_pixel = sum(im[0,:-1])//255 + sum(im[-1,1:])//255 + sum(im[1:,0])//255 + sum(im[:-1,-1])//255
  Worm_is_black = True if white_pixel/2/(im.shape[0]+im.shape[1])>0.5 else False
  return im.shape, Worm_is_black


def read_image_and_xy(imshape, filenames, rescale, plot_n, Worm_is_black):
  """read images and get skeletonized plots"""
  T = len(filenames)
  x = np.zeros((T, plot_n))
  y = np.zeros((T, plot_n))
  real_image = np.zeros((T,imshape[0],imshape[1]), dtype="uint8")
...
def read_image(imshape, filenames, rescale, Worm_is_black):
  real_image = np.zeros((len(filenames),imshape[0],imshape[1]), dtype="uint8")
  for t, f in enumerate(filenames):
    im = cv2.imread(f, cv2.IMREAD_GRAYSCALE)
    if Worm_is_black:
      im = 255 - im
...
def train3(model, real_image, optimizer, params, device, display_option, init_data, dataset_path, is_nont=True):
  T = real_image.shape[0]
  speed = params['speed']
  ...
  show_progress_freq = display_option['show_progress_freq']
  save_progress_freq = display_option['save_progress_freq']
  ...
  for e in range(epochs):
    ...
    if torch.min(annealing_weight) > 0.99:
      ...
      if early_stopping.early_stop:
        if display_option['ShowProgress']:
          print('Early stopping at epoch +{}.'.format(e-int(T/(2*speed))))
        break
    del loss
    optimizer.step()

    if display_option['ShowProgress']:   #Show Progress
      if e%show_progress_freq == 0:
        ...
        show_image(model_image, display_option['num_t'], title=f'epoch {e}')
    if e%save_progress_freq == 0:   #Save Progress
      save_progress(model_image, dataset_path, display_option, txt='id{}_{}'.format(display_option['id'],e))
  ...
  if display_option['ShowProgress']:   #Show Progress
    print('{:.2f} {:.2f} {:.2f} {:.2f}'.format(
        image_loss.item(),continuity_loss.item(),smoothness_loss.item(),length_loss.item(),center_loss.item()))
    show_image(model_image, display_option['num_t'], title='final')
  save_progress(model_image, dataset_path, display_option, txt='id{}_{}'.format(display_option['id'],'final'))
...
def judge_head(x, y):
  """Judge which tip is head."""
  theta = np.zeros((x.shape[0],x.shape[1]-1), dtype=float)
  for i in range(x.shape[1]-1):
    theta[:,i] = np.arctan2(y[:,i+1]-y[:,i], x[:,i+1]-x[:,i])
  curve_rate_var = (theta[:,1:] - theta[:,:-1]).var(axis=0)
  plt.plot(curve_rate_var)
  plt.xlabel('body segment', fontsize=20)
  plt.ylabel('curve rate var', fontsize=20)
  plt.show()
  curve_mean1 = curve_rate_var[:x.shape[1]//5].mean()
  curve_mean2 = curve_rate_var[-x.shape[1]//5:].mean()
...
def cancel_reduction(x, y, T_all, T_reduced, plot_n):
  if T_all == T_reduced:
    return x, y
  else:
    x_splined = np.zeros((T_all, plot_n))
    y_splined = np.zeros((T_all, plot_n))
    arclen = np.linspace(0, T_all, T_reduced)
    #interpolation
    div_linespace = np.linspace(0, T_all, T_all)
    for i in range(plot_n):
      f_x = interp1d(arclen, x[:,i], kind='linear')
      f_y = interp1d(arclen, y[:,i], kind='linear')
