...
class Model(torch.nn.Module):
    ...
    def forward(
        ...
        xy = F.pad(torch.cumsum(self.theta, dim=1), pad=(1, 0))
...
def train3(
    ...
    for e in range(epochs):
        ...
        # MSE for theta length
        theta_loss = torch.mean((torch.abs(model.theta) - 1.0) ** 2) * 1e6
        ...
        if T < 2:
            ...
            rotation_along_t = model.theta[:-1, :] * torch.conj(model.theta[1:, :])
    ...
    for e in range(params["epoch_plus"]):
        ...
        loss = image_loss + continuity_loss + smoothness_loss + length_loss + theta_loss
